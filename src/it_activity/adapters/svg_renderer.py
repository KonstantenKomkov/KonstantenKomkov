...
class SvgProfileRenderer:
    ...
    def __init__(self, intro: ProfileIntro | None = None) -> None:
        self._intro = intro if intro is not None else ProfileIntro.empty()
    ...
    def render(
        ...
        artifacts[README_PATH] = self._render_readme(artifacts)
        return artifacts
    ...
    def _render_readme(self, artifacts: Mapping[str, str]) -> str:
        lines = ["<!-- Generated by it-activity. Do not edit manually. -->", ""]
        if not self._intro.is_empty:
            lines.extend([*self._intro.markdown.split("\n"), ""])
        for period in SUPPORTED_PERIODS:
            open_attribute = " open" if period == DEFAULT_OPEN_PERIOD else ""
            commits_path = SvgProfileRenderer._versioned_path(commits_svg_path(period), artifacts)
            lines_path = SvgProfileRenderer._versioned_path(lines_svg_path(period), artifacts)
            lines.extend(
                [
                    f"<details{open_attribute}>",
                    f"<summary><strong>Последние {period} дней</strong></summary>",
                    "",
                    f"![Коммиты за {period} дней]({commits_path})",
                    "",
                    f"![Строки кода за {period} дней]({lines_path})",
                    "",
                    "</details>",
                    "",
                ]
            )
        usage_path = SvgProfileRenderer._versioned_path(USAGE_SVG_PATH, artifacts)
        lines.extend(
            [
                f"## Языки и технологии за {MAX_HISTORY_DAYS} дней",
                "",
                f"![Языки и технологии за {MAX_HISTORY_DAYS} дней]({usage_path})",
            ]
        )
        return "\n".join(lines) + "\n"

    @staticmethod
    def _versioned_path(path: str, artifacts: Mapping[str, str]) -> str:
        content = artifacts.get(path)
        if content is None:
            raise ProfileRenderingError("Не удалось связать README со всеми SVG профиля.")
        fingerprint = sha256(content.encode("utf-8")).hexdigest()[:12]
